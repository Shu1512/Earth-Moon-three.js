...
[こちら](https://github.com/Shu1512/Earth-Moon-three.js)githubのリンクです。
...
`scene`はミュージカルのステージだと考えるとわかりやすいと思います。
演者が立つステージを作成している感じです。
...
`camera`はミュージカルを収録している感じです。
上に書いているのは少なくとも何かしら撮らないとダメと言うことです。
...
ミュージカルを撮ったカメラをHDMIでテレビに繋げて見る感じです！
...
これでスポットライトを演者（3Dコンテンツ）に当ててを踊らすだけです💃。
...
次に球体を作成しますが、まずどの距離を撮るのかを決めます。
指定しないと0距離でミュージカルを収録しているので何をしているかわかりません。
なので100離れた場所からミュージカルを撮る処理を`camera`の下に記述を追加します。
...
scene.add(earth);
```
これでステージ、カメラ、演者、収録が準備できたので、演者を照らすスポットライトを作成します。
スポットライトがなくては何も見えないので設定していきましょう。
...
<img src="https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1729554/003a5e87-4b95-f3a0-6098-d80ea37533ce.png" width="360px"/>
これでステージ、カメラ、演者、スポットライト、収録の準備ができました。
次に衣装を変え振り付けをつけましょう！
